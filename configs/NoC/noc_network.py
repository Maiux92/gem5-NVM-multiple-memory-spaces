        ...
        for row in range(len(map)):
            for col in range(len(map[row])):
                ...
                if t_socket == 'n':
                    ...
                    nvm_ctrl            = MemCtrl()
                    #nvm_ctrl.nvm        = FRAM()
                    nvm_ctrl.nvm = eval("%s()" % options.nvm_type)
                    nvm_ctrl.nvm.range  = AddrRange(0x30000000, size="512MB")

                    s_dir = DirectorySocket(self, nvm_ctrl)
                ...
                if t_socket == 'v':
                    ...
                    mem_ctrl            = MemCtrl()
                    mem_ctrl.nvm        = SRAM()
                    mem_ctrl.nvm.range  = AddrRange("512MB")
